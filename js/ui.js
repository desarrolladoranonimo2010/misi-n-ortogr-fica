...
const AudioManager = {
  ...
  playConfigured(name, fallbackCorrect = null) {
    ...
    // Adelanto de 1 segundo para el sonido de victoria para evitar silencios iniciales
    clip.currentTime = (name === "victory") ? 1.0 : 0;
...
function renderGame() {
  ...
  // Marcado "Strict Lazy": Se registra solo lo que aparece en pantalla
  if (ch.word) Storage.markWordAsUsed(ch.word);
  if (ch.id && (ch.type === CHALLENGE_TYPES.EMAIL_CHECK || ch.type === CHALLENGE_TYPES.APP_CHECK || ch.type === CHALLENGE_TYPES.PROGRAMMING_CHECK || ch.type === CHALLENGE_TYPES.SECURITY_CHECK || ch.type === CHALLENGE_TYPES.DETECT_ERROR || ch.type === CHALLENGE_TYPES.MEANING)) {
    Storage.markTextAsUsed(ch.id);
  }
...
function finalizeSession(timedOut = false) {
  ...
  // PRIORIDAD: Reproducir sonido inmediatamente para evitar delay percibido
...
/* =======================================================================
   ...
   ======================================================================= */
function startQuick() {
  const q = { current: null, score: 0, correct: 0, wrong: 0, timeLeft: 60, timerId: null, ended: false, bag: new ShuffleBag(typeof WORDS !== 'undefined' ? WORDS : []) };
...
function startTwoPlayer(name1, name2) {
  ...
  const bag = new ShuffleBag(typeof WORDS !== 'undefined' ? WORDS : []);
  State.two = {
    bag: bag,
    current: pickFor1v1("normal", bag),
    index: 0,
    turn: 1,
    scores: { 1: 0, 2: 0 },
...
function twoRoundLimit(t) {
  if (t.phase === "extreme") return 5;
  return [60, 45, 30, 20, 10][Math.min(t.suddenRound, 5) - 1];
}
...
function twoAnswer(value, timedOut = false) {
  ...
  // Registrar estadísticas globales incluso en modo PvP
  Storage.recordAnswer({ 
    correct, 
    letter: ch.letter, 
    isException: ch.isException, 
    ruleId: ch.ruleId, 
    context: "pvp" 
  });
...
function twoNext() {
  ...
  if (t.phase === "normal" && t.index >= t.rounds) {
    ...
    if (t.scores[1] !== t.scores[2]) {
      ...
      if (t.phase === "sudden" && t.suddenRound < 5) {
        t.suddenRound++;
        t.turn = 1;
        t.announcement = "Hora del desempate: el margen de error se reduce.";
      } else if (t.phase === "sudden") {
        t.phase = "extreme";
        t.suddenRound = 1;
        t.suddenAnswers = 0;
        t.turn = 1;
        t.announcement = "MUERTE SÚBITA EXTREMA: 5 segundos. El primero en fallar, pierde.";
      } else {
        t.suddenRound++;
        t.turn = 1;
        t.announcement = "Máxima presión: la batalla ortográfica continúa.";
      }
    }
  }

  if (!t.ended) {
    t.current = pickFor1v1(t.phase, t.bag);
  }
