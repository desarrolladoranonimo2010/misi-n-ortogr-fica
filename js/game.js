...
/* -------------------------------------------------------------------------
   Pool de palabras, Shuffle Bag y Selección Adaptativa
   ------------------------------------------------------------------------- */
class ShuffleBag {
  ...
  next(minD, maxD, onlyExceptions = false) {
    if (this.bag.length === 0) return null;

    for (let i = this.index; i < this.bag.length; i++) {
      const w = this.bag[i];
      const inDifficulty = w.difficulty >= minD && w.difficulty <= maxD;
      const isEx = !!w.isException;
      
      if ((!onlyExceptions || isEx) && inDifficulty) {
        const selected = this.bag[i];
        this.bag[i] = this.bag[this.index];
        this.bag[this.index] = selected;
        this.index++;
        return selected;
      }
    }

    const hasAnyMatching = this.allWords.some(w => {
      const inDifficulty = w.difficulty >= minD && w.difficulty <= maxD;
      return (!onlyExceptions || !!w.isException) && inDifficulty;
    });

    if (!hasAnyMatching) {
      if (onlyExceptions) return this.next(minD, maxD, false);
      this.reset();
      const fallback = this.bag[this.index];
      this.index++;
      return fallback;
    }

    this.reset();
    return this.next(minD, maxD, onlyExceptions);
  }
}
...
function pickAdaptiveQuickChallenge(quickState) {
  ...
  let item;
  if (quickState.bag) {
    item = quickState.bag.next(minD, maxD);
  } else {
    const pool = GlobalPools.getShared();
    item = pool.nextAdaptive(minD, maxD);
  }
