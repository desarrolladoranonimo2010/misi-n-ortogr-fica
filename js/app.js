...
document.addEventListener("keydown", (ev) => {
  ...
  // Soporte para teclas G y J en desafíos de selección
  if (State.screen === "game" || State.screen === "quick" || State.screen === "two") {
    if (!State.answered) {
      const key = ev.key.toUpperCase();
      if (key === "G" || key === "J") {
        // Solo actuar si hay botones de G o J visibles
        const btn = document.querySelector(`.letter-btn[data-value="${key}"]`);
        if (btn) {
          ev.preventDefault();
          btn.click();
        }
